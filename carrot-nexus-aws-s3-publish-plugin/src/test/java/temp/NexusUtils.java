...
class NexusUtils {
	...
	static File retrieveFile(final Repository repository, final String path)
		...
		final File content = ((DefaultFSLocalRepositoryStorage) repository
				.getLocalStorage()).getFileFromBase(repository, request);
	...
	static StorageItem retrieveItem(final Repository repository,
	...
	static StorageItem safeRetrieveItem(final Repository repository,
			final String path) {
		try {
	...
	static void storeItem(final Repository repository,
			final ResourceStoreRequest request, final InputStream in,
			final String mimeType, final Map<String, String> userAttributes)
			throws Exception {

		final DefaultStorageFileItem fItem = new DefaultStorageFileItem(
				repository, request, true, true, new PreparedContentLocator(in,
						mimeType));
	...
	static void createLink(final Repository repository, final StorageItem item,
			final String path) throws Exception {
		...
		final DefaultStorageLinkItem link = new DefaultStorageLinkItem(
				repository, request, true, true, item.getRepositoryItemUid());
	...
	static File localStorageOfRepositoryAsFile(final Repository repository)
		...
		if (repository.getLocalUrl() != null
			...
			final File baseDir = ((DefaultFSLocalRepositoryStorage) repository
					.getLocalStorage()).getBaseDir(repository,
		...
		throw new LocalStorageException(String.format(
				"Repository [%s] does not have an local storage",
				repository.getId()));
	...
	static boolean isHidden(final String path) {
		...
		return path.startsWith(DOT) || path.startsWith("/" + DOT)
				|| path.startsWith(File.separator + DOT);
	}

	static String[] getReversePathSegments(final File file) {
